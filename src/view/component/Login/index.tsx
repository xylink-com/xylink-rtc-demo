...
interface IProps {
  user: IUser,
  isThird?: boolean,
  onChangeUserInfo: (value: any, tyep: string) => void;
  onHandleSubmit: () => Promise<void>;
  onToggleSetting: () => void;
}

const Login = (props: IProps) => {
  const { user, isThird = THIRD, onChangeUserInfo, onHandleSubmit, onToggleSetting } = props;
          ...
          <Form onFinish={onHandleSubmit} className="login-form" initialValues={user}>
            {
              ...
              isThird ?
                <>
                  <Form.Item
                    name="extUserId"
                    rules={[{ required: true, message: '请输入第三方用户ID' }]}
                  >
                    <Input
                      type="extUserId"
                      placeholder="第三方用户ID"
                      onChange={(e) => {
                        onChangeUserInfo(e.target.value, 'extUserId')
                      }}
                    />
                  </Form.Item>
                </> :
                <>
                  ...
                  >
                    <Input
                      ...
                      placeholder="输入小鱼账号"
                      onChange={(e) => {
                        onChangeUserInfo(e.target.value, 'phone')
                      }}
                    />
                  </Form.Item>
                  <Form.Item
            ...
            >
              <Input
                ...
                autoComplete='off'
                onChange={(e) => {
                  onChangeUserInfo(e.target.value, 'meeting')
                }}
              />
            </Form.Item>
            <Form.Item
            ...
            >
              <Input
                ...
                autoComplete='off'
                onChange={(e) => {
                  onChangeUserInfo(e.target.value, 'meetingPassword')
                }}
              />
            </Form.Item>
            ...
            >
              <Input
                ...
                autoComplete='off'
                onChange={(e) => {
                  onChangeUserInfo(e.target.value, 'meetingName')
                }}
              />
            </Form.Item>
            ...
            <Form.Item name="muteVideo" style={{ marginBottom: '15px' }}>
              <Checkbox
                ...
                onChange={(e) => {
                  onChangeUserInfo(e.target.checked, 'muteVideo')
                }}
              >
                入会时关闭摄像头
              </Checkbox>
            ...
            <Form.Item name="muteAudio">
              <Checkbox
                ...
                onChange={(e) => {
                  onChangeUserInfo(e.target.checked, 'muteAudio')
                }}
              >
                入会时静音
              </Checkbox>
